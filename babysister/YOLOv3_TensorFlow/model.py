...
class yolov3(object):
    ...
    def loss_layer(self, feature_map_i, y_true, anchors):
        ...
        valid_true_boxes = tf.boolean_mask(
            y_true[..., 0:4], tf.cast(object_mask[..., 0], "bool")
        )
        ...
        # calc iou
        # shape: [N, 13, 13, 3, V]
        iou = self.broadcast_iou(
            valid_true_box_xy, valid_true_box_wh, pred_box_xy, pred_box_wh
        )
        ...
        true_tw_th = tf.where(
            condition=tf.equal(true_tw_th, 0), x=tf.ones_like(true_tw_th), y=true_tw_th
        )
        pred_tw_th = tf.where(
            condition=tf.equal(pred_tw_th, 0), x=tf.ones_like(pred_tw_th), y=pred_tw_th
        )
        ...
        # shape: [N, 13, 13, 3, 1]
        xy_loss = (
            tf.reduce_sum(tf.square(true_xy - pred_xy) * object_mask * box_loss_scale)
            / N
        )
        wh_loss = (
            tf.reduce_sum(
        ...
        class_loss = object_mask * tf.nn.sigmoid_cross_entropy_with_logits(
            labels=y_true[..., 5:], logits=pred_prob_logits
        )
